...
utils.die = function (reason, code) {
  console.log('');
  console.log(clc.red('  Error: %s'), reason);
  process.exit(code);
};
 ...
 * @return {String}
 */
utils.getProcessStatus = function (info) {
  info = info || {};
  ...
  let list = [];
  for (const i in proc) list.push(`${i}:${proc[i]}`);
  for (const i in mem) list.push(`${i}:${mb(mem[i])}`);
  for (const i in info) list.push(`${i}:${info[i]}`);
  return list.join(', ');
};
